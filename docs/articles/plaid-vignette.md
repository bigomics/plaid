# PLAID: ultrafast single-sample enrichment scoring

## Introduction

We developed PLAID (Pathway Level Average Intensity Detection), a novel,
ultrafast and memory optimized gene set scoring algorithm. PLAID
...
Note that X is the normalized expression matrix form the Seurat object,
...
the column ‘p.meta’ and adjusted for multiple testing in column
`q.dual`.
...
![](plaid-vignette_files/figure-html/unnamed-chunk-7-1.png)

## Replicating ssGSEA, singscore and scSE

Plaid can be used to replicated the singscore (Fouratan et al., 2018)
and Single-Cell Signature Explorer (scSE) score (Pont et al., 2019). But
using plaid, the computation is much faster than the original code.
...
and from the original `GSVA` R package. Note the rank weight parameter
...
not exactly the same. The default value in the original publication and
in GSVA is `alpha=0.25`.
...
![](plaid-vignette_files/figure-html/unnamed-chunk-11-1.png)
