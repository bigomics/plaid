...
pdf("compare-plaid-vs-sing.pdf", h=16, w=9, pointsize=14)
...
for(ds in DATASETS) {
  ...
  plot( G1[,1], G2[,1], xlab="singscore", ylab="replaid.sing")
  title("score", cex.main=1.2)
  legend("topleft", legend=toupper(ds), cex=0.8, bty="n")
  
  plot( m1$logFC, m2$logFC, xlab="singscore", ylab="replaid.sing" )
  title("logFC", cex.main=1.2)
  plot( m1$P.Value, m2$P.Value, xlab="singscore", ylab="replaid.sing" )
  title("p-value", cex.main=1.2)
  plot( -log10(m1$P.Value), -log10(m2$P.Value), xlab="singscore", ylab="replaid.sing" )
